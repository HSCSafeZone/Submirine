...
public class SafeZoneServer extends JFrame {
    ...
    class Client extends Thread {
        ...
        private void processMessage(String msg) {
            ...
            } else if ("SURRENDER".equals(msg)) {
                handleSurrenderRequest(this);
            } else {
                handleOtherMessages(msg);
            }
        }

        private void handleSurrenderRequest(Client surrenderingClient) {
            synchronized (clients) {
                for (Client client : clients) {
                    if (client != surrenderingClient) {
                        client.sendMessage("OPPONENT_SURRENDERED");
                        client.getPlayer().addWin(); // 상대방이 항복했으므로 승리
                        client.sendPlayerInfo(); // 승리 정보 업데이트
                    }
                }
                surrenderingClient.getPlayer().addLoss(); // 항복한 클라이언트는 패배
                surrenderingClient.sendPlayerInfo(); // 패배 정보 업데이트
                surrenderingClient.sendMessage("YOU_SURRENDERED");
        ...
        private void sendGameOver() {
            ...
            for (Client client : clients) {
                if (client.score > maxScore) {
                    ...
                    winnerName = client.getPlayer().getName();
            ...
            if (isDraw || maxScore == 0) {
                ...
                for (Client client : clients) {
                    if (client.getPlayer().getName().equals(winnerName)) {
                        client.getPlayer().addWin();
                    } else {
                        client.getPlayer().addLoss();
                    }
